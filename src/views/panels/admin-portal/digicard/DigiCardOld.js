...
const AddDigiCard = (
  ...
  const [isFileTypeImg, setFiletypeImg] = useState();
  const [isFileTypeAudio, setFiletypeAudio] = useState();
  ...
  const validateFileTypeImg = (e) => {
    if (areFilesInvalid([e.target.files[0]]) !== 0) {
      sweetAlertHandler(
        {
          title: 'Invalid Image File(s)!', type: 'warning',
          text: 'Supported file formats are .png, .jpg, .jpeg. Uploaded files should be less than 2MB. '
        }
      );
    } else {
      setFiletypeImg(e.target.files[0])
    }
  }

  const validateFileTypeAudio = (e) => {
    if (voiceInvalid([e.target.files[0]]) !== 0) {
      sweetAlertHandler(
        {
          title: 'Invalid Audio File(s)!',
          type: 'warning', text: 'Supported file formats are .mp3, .mpeg, .wav. Uploaded files should be less than 10MB '
        }
      );
    } else {
      setFiletypeAudio(e.target.files[0])
    }
  }
          ...
          <Card.Body>
            ...
            <Formik
              ...
              onSubmit={async (values, { setErrors, setStatus, setSubmitting }) => {
                ...
                if (isFileTypeImg === '' || isFileTypeImg === undefined || isFileTypeImg === null) {
                  setImgValidation(false)
                  hideLoader();
                } else if (voiceNote !== undefined && (isFileTypeAudio === '' || isFileTypeAudio === undefined || isFileTypeAudio === null)) {
                  setVoiceError(false)
                  ...
                  var formData = {
                    ...
                    digicard_voice_note: isFileTypeAudio === undefined || isFileTypeAudio === '' || isFileTypeAudio === null ? "" : isFileTypeAudio,
