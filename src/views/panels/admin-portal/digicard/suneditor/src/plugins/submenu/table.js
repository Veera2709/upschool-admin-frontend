...
export default {
    ...
    setController_table: function (core) {
        ...
        tableResize.innerHTML = '' +
            ...
            icons.table_header +
            '<span class="se-tooltip-inner"><span class="se-tooltip-text">' + "Horizontal Table Header" + '</span></span>' +    
            '</button>' +
            '<button type="button" data-command="insert" data-value="cell" data-option="left1" class="se-btn se-tooltip _se_table_header">' +
            icons.insert_column_left +
            '<span class="se-tooltip-inner"><span class="se-tooltip-text">' + "Add Vertical Table Header" + '</span></span>' +
            '</button>' +
    ...
    appendTable: function () {
        ...
        var borderSize = document.getElementById("tableWidth").value;
        ...
        let tableHTML = '<tbody style="border: '+borderSize+'px solid black;">';
        while (y > 0) {
    ...
    createCells: function (nodeName, cnt, returnElement) {
       ...
       var borderSize =  sessionStorage.getItem('borderSize')
       console.log("borderSize",borderSize);
        nodeName = nodeName.toLowerCase();
        if (!returnElement) {
            let cellsHTML = '';
            while (cnt > 0) {
                cellsHTML += '<' + nodeName + ' style="border: '+borderSize+'px solid black;"><div><br></div></' + nodeName + '>';
                cnt--;
            }
            return cellsHTML;
        } else {
    ...
    onClick_tableController: function (e) {
        ...
        switch (command) {
            ...
            case 'header':
                ...
                break;
            case 'headerCol':
                tablePlugin.toggleHeaderCol.call(this);
                break;
            case 'onsplit':
