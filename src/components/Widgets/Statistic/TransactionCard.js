  ...
  return (
    <Card className="trnasiction-card">
      <Card.Header className={params.headerClass}>
        ...
        <div className="card-header-right">{params.headerRight}</div>
      </Card.Header>
      <Card.Body className="text-center">
        ...
        <h6>
          Registered Respiratory <span className="d-block">Therapist</span>
        </h6>
        <p>RT 334445566</p>
      </Card.Body>
      <Row className="border-top transection-footer row">
        <Col className="border-right">
          <Link to={params.leftLink.to} className="text-dark">
            <i className={leftIconClass.join(' ')} />
            {params.leftLink.label}
          </Link>
        </Col>
        <Col>
          <Link to={params.rightLink.to} className="text-dark">
            <i className={rightIconClass.join(' ')} />
            {params.rightLink.label}
          </Link>
        </Col>
      </Row>
    </Card>
